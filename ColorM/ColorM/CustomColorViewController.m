...
@interface CustomColorViewController ()<GDTMobInterstitialDelegate>
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (isBuy == 0) {
        ...
        self.bannerView = [[GDTMobBannerView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-self.tabBarController.tabBar.frame.size.height-50, self.view.frame.size.width, 50)
                                                           appkey:GDT_APPID placementId:GDT_BANNER_FAV];

        self.bannerView.currentViewController = [[UIApplication sharedApplication] keyWindow].rootViewController;
    ...
    self.interstitial = [[GDTMobInterstitial alloc] initWithAppkey:GDT_APPID placementId:GDT_INTERSTITIAL_CUSTOMFAV];
